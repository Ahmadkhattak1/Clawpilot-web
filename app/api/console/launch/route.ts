...
interface InternalConsoleLaunchResponse {
  launch?: {
    url?: unknown
...
export async function POST(request: Request) {
  ...
  try {
    backendResponse = await fetch(
      `${backendApiUrl}/api/v1/internal/tenants/${encodeURIComponent(tenantId)}/console/launch`,
      {
        method: 'GET',
        headers: {
  ...
  const backendPayload = (await parseJsonSafe(backendResponse)) as InternalConsoleLaunchResponse | null
  if (!backendResponse.ok) {
    return NextResponse.json(
      {
        error: readString(backendPayload?.error) ?? 'CONSOLE_LAUNCH_UNAVAILABLE',
        message: readString(backendPayload?.message) ?? 'Unable to open console right now.',
      },
    ...
    return NextResponse.json(
      {
        ...
        message: 'Backend console launch payload is missing launch URL.',
      },
      { status: 502 },
    )
  }
  ...
  if (!normalizedLaunchUrl) {
    return NextResponse.json(
      {
        error: 'INVALID_BACKEND_RESPONSE',
        message: 'Backend console launch URL is not valid.',
      },
      { status: 502 },
    )
  }
